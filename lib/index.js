 ...
 */
const extractClassroom = data => data.split('/').map(element => (element.match(/\((.+)\)/) || [null, null])[1]).filter(Boolean)

/**
 * 모든 학과의 이름과 코드를 반환합니다.
 * @returns {Object.<string, string>} 학과정보
 */
const getAll = () => deptData;
 ...
 * @returns {?string} 학과코드
 */
const getDeptCode = deptName => deptData[deptName] || (() => {
  console.error(`* ${deptName}는 올바른 과/학부 이름이 아닙니다.`)
  console.info(`* 목록: [${Object.keys(deptData).join(' ')}]`)
  return null
})()
 ...
 * @param {string} deptCode 학과코드
 * @param {string} category 과목구분
 * @returns {Promise.<Array.<Object>>} 강의목록
